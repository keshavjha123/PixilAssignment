...
/**
 * Get image build history for a DockerHub image tag.
 * Implements unauthorized→authorized fallback pattern.
 */
export async function getImageHistory(namespace: string, repository: string, tag: string, token?: string): Promise<ImageHistory[]> {
    ...
    try {
        ...
        return await fetchImageHistoryWithToken(repo, tag, registryToken);
    } catch (error: unknown) {
        if (error instanceof AxiosError && (error.response?.status === 401 || error.response?.status === 404) && token) {
            ...
            try {
                ...
                try {
                    ...
                    return await fetchImageHistoryWithToken(repo, tag, bearerToken);
                }
            } catch (authError) {
...
async function fetchImageHistoryWithToken(repo: string, tag: string, token: string): Promise<ImageHistory[]> {
